...
def replicate_input_fn(input_fn, devices=None):
    if not devices:
        devices = _get_local_devices('GPU')
    if not devices:
        raise ValueError("Coudln't find local GPU devices")

    def _replicated_input_fn():
        result = input_fn()
        if not isinstance(result, dataset_ops.Dataset):
            raise ValueError(
                "input_fn should return an object of"
                "type Dataset for it to be replicated")
        return values.PerDeviceDataset(result, devices, True)
    return _replicated_input_fn
...
def replicate_model_fn(model_fn,
  ...
  return _replicate_model_fn_with_mode(
      ...
      # appropriate.
      # mode=_VariableDistributionMode.SHARED_ROUND_ROBIN)
...
def _replicate_model_fn_with_mode(
  ...
  def replicated_model_fn(features, labels, mode, params=None, config=None):
    ...
    if not isinstance(labels, values.PerDevice):
        ...
        feature_shards, label_shards = zip(*(values.select_device(device,
                                                    (features, labels))
...
class TowerOptimizer(optimizer_lib.Optimizer):
  ...
  def __init__(self, optimizer_or_optimizer_fn, devices=None, consolidation_device="/device:CPU:0"):
    ...
    self._devices = devices or _get_local_devices('GPU')
    self._consolidation_device = consolidation_device
  ...
  def _apply_gathered_gradients(self, global_step, **kwargs):
    ...
    with ops_lib.device(self._consolidation_device):
